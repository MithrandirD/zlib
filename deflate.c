...
static void slide_hash(s)
    deflate_state *s;
{
    /* s->w_size would always be a PoT */
    z_const  uint32_t wchunks = (s->w_size >> 3);
    ...
    /* s->hash_size would always be a PoT */
    n = s->hash_size >> 3;
    q = (__m128i *)s->head;
    do {
        _mm_storeu_si128(q++, _mm_subs_epu16(_mm_loadu_si128(q), xmm_wsize));
    } while (--n > 0);
